...
class Calculate(QMainWindow, untitled.Ui_MainWindow):
    ...
    def finnedTube(self):
        if self.lineEdit_13.text() == '' or self.lineEdit_14.text() == '' or \
            ...
            delta_heat_temper = delta_aver_temper*self.doubleSpinBox.value()
            # print('传热温差：%.2f' % delta_heat_temper)
            self.textBrowser_2.append('传热温差：%.2f ℃' % delta_heat_temper)
            heat_trans = self.cap_heat_trans
            area_heat = self.doubleSpinBox_2.value()*heat_trans/(tube_heat*delta_heat_temper)
            ...
            # 换热面积裕度
            area_margin = (area_heat_true/area_heat - 1)*100
            self.textBrowser_2.append('面积裕度：%.2f' % area_margin + '%')
            press_drop_smoke = drag_coefficient_1 * self.fin_vert_num      # 烟气压力降(Pa)
            # print("烟气压力降：%.2f" % press_drop_smoke)
            self.textBrowser_2.append("烟气压力降：%.2f Pa" % press_drop_smoke)
            self.textBrowser_2.append('-----------------------------')
    ...
    def TubeInHeatCoefficient(self):
        ...
        if cooler_Re > 10000:
            ...
            press_drop_cooler = drag_coefficient_cooler*area_width*cooler_D*cooler_v**2/(2*(tube_in_diameter*1e-3))
        else:
            cooler_heat_coefficient = 1.86*(cooler_L/(tube_in_diameter*1e-3))*(cooler_Re*cooler_Pr)**(1/3)*(cooler_U/cooler_U_wall)**0.14
            press_drop_cooler = 32*area_width*cooler_U*cooler_v/((tube_in_diameter*1e-3)**2)
            # 管内换热系数
        ...
        self.textBrowser_2.append("管内压力降：%.2f Pa" % press_drop_cooler)
...
class Physical_property_parameter_smoke(QWidget, untitled.Ui_MainWindow):
    def __init__(self):
        ...
        self.textBrowser_3.setGeometry(QtCore.QRect(0, 0, 280, 260))
...
class Physical_property_parameter_cooler(QWidget, untitled.Ui_MainWindow):
    def __init__(self):
        ...
        self.textBrowser_4.setGeometry(QtCore.QRect(0, 0, 280, 260))
